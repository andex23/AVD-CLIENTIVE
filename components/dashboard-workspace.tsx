...
import { getDashboardPath, type DashboardWorkspaceSection } from "@/lib/dashboard-routes"
...
function DashboardWorkspaceContent({ active }: { active: DashboardWorkspaceSection }) {
  ...
  const focusQueue = useMemo(() => {
    ...
    return [...overdueTasks, ...todaysTasks, ...nextTasks]
      .filter((task) => {
  ...
  const navigate = (key: AppNavKey) => {
    router.push(getDashboardPath(key, isPreview))
  }
  ...
  const status =
    active === "clients" ? (
      ...
      <p>
        {overdueTasks.length} overdue, {todaysTasks.length} due today.
      </p>
    ) : active === "orders" ? (
      <p>
      ...
      <p>
        {activeClientsCount} active clients. {warmClientsCount} warm leads still in motion.
      </p>
    )
      ...
      >
        {active === "today" ? (
          <div className="space-y-8">
            ...
            <div className="grid gap-7 xl:grid-cols-[minmax(0,1.16fr)_348px]">
                ...
                <div className="space-y-2 px-7 py-6">
                  {focusQueue.length > 0 ? (
                    focusQueue.map((task) => {
                      ...
                      return (
                        <div key={task.id} className="border-b border-border/80 px-3 py-5 last:border-b-0">
                                  ...
                                  className={
                                    isOverdue(task.dueDate, task.completed)
                                      ...
                                      : isToday(task.dueDate)
                                        ...
                                        : "border-border/80 bg-[hsl(var(--surface-soft))] text-muted-foreground"
                                  }
                                >
              ...
              <aside className="space-y-6 border-t border-border pt-6 xl:border-l xl:border-t-0 xl:pl-12 xl:pt-0">
                <section className="space-y-4">
                  ...
                  {attentionClients.length > 0 ? (
                    attentionClients.map((client) => {
                      ...
                      return (
                        <div key={client.id} className="border-t border-border py-5 first:border-t-0">
                          <div className="flex items-start justify-between gap-3">
                            ...
                            <span className={`rounded-full border px-3 py-1.5 text-[11px] font-medium ${attention.tone}`}>{attention.label}</span>
                          </div>
                        </div>
                      )
                    })
                ...
                <section className="space-y-4 border-t border-border pt-6">
                  ...
                  {nextTasks.length > 0 ? (
                    nextTasks.map((task) => (
                      <div key={task.id} className="border-t border-border py-5 first:border-t-0">
                        <p className="font-medium text-foreground">{task.title}</p>
                        <p className="ui-meta mt-1">
                          {getClientName(task.clientId, clients)} · {formatEnumLabel(task.type)}
                        </p>
                        <p className="ui-meta mt-2">
                          {formatDateShort(task.dueDate)} at {formatTimeHM(task.dueDate)}
                        </p>
                      </div>
                    ))
                  ) : (
                ...
                <section className="space-y-4 border-t border-border pt-6">
                  ...
                  {activityFeed.map((item) => (
                    <div key={item.id} className="border-t border-border py-5 first:border-t-0">
                  ...
                  {settings.orderTrackingEnabled ? (
                    <>
                      ...
                      <div className="border-t border-border pt-5">
                        {recentOrders.map((order) => (
                          <div key={order.id} className="border-t border-border py-4 first:border-t-0">
                            <p className="text-sm font-medium text-foreground">{order.product}</p>
                            <p className="ui-meta mt-1">
                              {getClientName(order.clientId, clients)} · ${order.amount.toFixed(0)}
                            </p>
                          </div>
                        ))}
                      </div>
          ...
          <div className="space-y-5">
            <MetricsGrid>
              ...
              <MetricTile
                icon={Package2}
                label="Order history"
                value={settings.orderTrackingEnabled ? "On" : "Off"}
                detail={settings.orderTrackingEnabled ? "Visible inside client records" : "Still optional"}
              />
            </MetricsGrid>
          ...
          <div className="space-y-5">
            ...
            <TaskList tasks={tasks} clients={clients} isPreview={isPreview} />
          </div>
        ) : settings.orderTrackingEnabled ? (
          <div className="space-y-5">
            <MetricsGrid>
            ...
            <div className="grid gap-5 p-8 lg:grid-cols-[0.7fr_0.3fr] lg:items-center">
              ...
              <div className="flex flex-wrap gap-2 lg:justify-end">
                <Button variant="outline" asChild>
                  <Link href={getDashboardPath("settings", isPreview)}>Open settings</Link>
                </Button>
...
function MetricsGrid({ children }: { children: React.ReactNode }) {
  return <div className="grid gap-6 border-y border-border py-6 sm:grid-cols-2 xl:grid-cols-4">{children}</div>
}
...
export function DashboardWorkspaceRoute({ active }: { active: DashboardWorkspaceSection }) {
  return (
    <SettingsProvider>
      <OrdersProvider>
        <TasksProvider>
          <ClientsProvider>
            <DashboardWorkspaceContent active={active} />
          </ClientsProvider>
        </TasksProvider>
      </OrdersProvider>
    </SettingsProvider>

      ...
      actions={
        <>
          <Button variant="outline" asChild>
            <Link href={getDashboardPath("today", isPreview)}>Back to workspace</Link>
          </Button>
          <Button asChild>
...
function FastPath({ href, title, body }: { href: string; title: string; body: string }) {
  return (
    <Link href={href} className="flex items-center justify-between border-b border-border px-2 py-4 transition-colors hover:text-foreground">
      <div>
        <p className="text-sm font-medium text-foreground">{title}</p>

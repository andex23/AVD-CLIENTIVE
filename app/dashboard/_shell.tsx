    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="border-b">
        <div className="flex h-16 items-center px-4">
          <div className="flex items-center space-x-3">
            <img src="/brand/logo-mark.png" alt="AVD Clientive" className="h-6 w-6" />
            <h1 className="text-xl font-semibold font-mono">AVD Clientive</h1>
          </div>
          <div className="ml-auto flex items-center space-x-2 md:space-x-4">
            <div className="hidden md:flex items-center space-x-2">
              <Switch id="order-tracking" checked={settings.orderTrackingEnabled} onCheckedChange={(checked) => updateSettings({ orderTrackingEnabled: checked })} />
              <Label htmlFor="order-tracking" className="text-sm font-sans">Order Tracking</Label>
            </div>
            <div className="relative">
              <Search className="absolute left-2 top-2.5 h-4 w-4 text-muted-foreground" />
              <Input placeholder="Search clients..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-8 w-[240px] md:w-[300px]" />
            </div>
      ...
      <div className="flex-1 space-y-4 p-4 pt-6">
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <Card className={`${settings.orderTrackingEnabled ? "" : "opacity-50"}`}>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Inactive Customers</CardTitle>
              <Bell className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-orange-600">{settings.orderTrackingEnabled ? stats.inactiveCustomers : "-"}</div>
      ...
      {settings.orderTrackingEnabled && <AddOrderDialog open={showAddOrder} onOpenChange={setShowAddOrder} clients={clients} />}
      ...
      <FilterDialog open={showFilter} onOpenChange={setShowFilter} onFilter={filterClients} orderTrackingEnabled={settings.orderTrackingEnabled} />
    </div>
  )
}
